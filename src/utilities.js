...
function sortIngredients(ingredients){
    const copyIngred = [...ingredients]
    return copyIngred.sort(compareIngredientsCB)
}
...
function isKnownTypeCB(type){
    const allTypes = Object.keys(dishTypeRanking) ;
...
function dishType(dish){
    ...
    if(dish.dishTypes === undefined){
        ...
        const foundDishType = dish.dishTypes.find(isKnownTypeCB)
        if(foundDishType === undefined){
            return "";
        }
...
function __give_a_proper_name_CB(dishA, dishB){
    ...
    const intA = dishTypeRanking[typeA]
    const intB = dishTypeRanking[typeB]
...
function sortDishes(dishes){
    ...
    return copyDishes.sort(__give_a_proper_name_CB);
}
...
function menuPrice(dishesArray){
    ...
    function calcSum(param1, param2){
    ...
    const priceArray = dishesArray.map(getDishPrice)
    ...
    return priceArray.reduce(calcSum,0)
...
export {compareIngredientsCB, sortIngredients, isKnownTypeCB, dishType, __give_a_proper_name_CB, sortDishes, shoppingList,menuPrice};
